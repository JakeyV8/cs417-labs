...
Time: ~30 minutes File to edit: hash_table.py Test command: pytest -v

Background
A hash table stores key-value pairs and provides fast (average O(1)) lookups. It works in three steps:

Hash the key → produces a number
Modulo that number by the table size → gives a bucket index
...
Use _hash() to find the right bucket
Loop through the bucket to find the matching key
Return the value if found
Raise KeyError(key) if not found
...
Use _hash() to find the right bucket
Loop through the bucket with enumerate() to get the index
If you find the key, use del or .pop() to remove it, then decrement self.count
If the key isn't found, raise KeyError(key)
