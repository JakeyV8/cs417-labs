...
Time: ~45 minutes Starter files: hash_table_open.py, test_hash_table.py, test_tombstones.py, conftest.py Test command: pytest -v (from your Lab9 root)
...
Move hash_table_open.py into src/
Move both test files into tests/
Keep conftest.py in the Lab9 root (it tells pytest where to find your code)
Verify: run pytest -v from the Lab9 root — all tests should fail (that's correct, you haven't implemented anything yet)
Commit checkpoint:
...
Compute the starting index with _hash(key)
Walk forward through the table (wrapping around with %), checking each slot:
...
If you've checked every slot and found no room → raise Exception("Hash table is full")
...
If you've checked every slot → raise KeyError
...
Start at _hash(key), probe forward (same as get)
If you find the matching key → replace the slot with _TOMBSTONE, decrement count
If you hit None or exhaust the table → raise KeyError
...
Probe chain survives deletion — insert keys that collide, delete one in the middle, verify you can still find the one after it
Tombstone slot gets reused — delete a key, insert a new key that would probe to the same slot, verify the table size didn't grow unnecessarily
Count stays correct — delete and reinsert keys, check len() at each step
...
Working in Colab
First thing: Click File → Save a copy in Drive. This gives you your own editable copy. Without this step, your work will disappear when you close the tab.
Paste your code: Copy your completed HashTableOpen class (including the _TOMBSTONE = object() line) from src/hash_table_open.py and paste it into the first code cell.
Run each cell and answer the questions in the markdown cells.
What You'll Explore
The notebook has three experiments:

Collision counting — how many probes does it take to insert items as the table fills up? You'll see probe counts spike after ~75% full.
Table visualization — see your table as a color-coded bar chart (green = occupied, red = tombstone, gray = empty). Watch clusters form.
Chaining vs probing — side-by-side performance comparison. Which handles high load factors better, and why?
Saving Back to GitHub
When you're done, save the notebook back to your repo:

File → Save a copy in GitHub
Colab will ask you to authorize with GitHub (one-time setup)
Select your repo and set the path to labs/Lab9/notebooks/analysis.ipynb
Click OK — Colab commits it directly to your repo
